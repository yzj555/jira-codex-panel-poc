# jira-codex-panel-dsh

DeepSeek Harness 壳：把宿主无关的 Jira/JXL/SVN 业务核（`@jira-codex/core`）接入 DSH。
...
1. 启动 core 独立服务（默认 `127.0.0.1:47823`）：
   ...
   core 服务复用 `%LOCALAPPDATA%\jira-codex-panel-poc\` 下的 Jira 配置与 SVN 状态文件，与 Codex 壳共享同一套数据；若 Codex 壳正在运行则需改用其他端口并设置 `JIRA_CODEX_CORE_URL`。
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `JIRA_CODEX_CORE_URL` | `http://127.0.0.1:47823/mcp` | mcp-client 连接的 core 服务地址 |
| `JIRA_POC_HOST` / `JIRA_POC_PORT` | `127.0.0.1` / `47823` | core 独立服务绑定地址/端口 |
