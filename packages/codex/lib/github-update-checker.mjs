...
function normalizeReleaseAssets(value) {
  return (Array.isArray(value) ? value : [])
    ...
    .filter((asset) => asset.name && /^https:\/\/github\.com\//i.test(asset.url));
}
...
export function createGitHubUpdateChecker({
  ...
  releaseManifestUrl = `https://github.com/${repository}/releases/latest/download/update-manifest.json`,
  ...
  async function readLatestVersion() {
    ...
    try {
      ...
      if (latestVersion) {
        ...
        return {
          ...
          assets,
          ...releaseInstallability(assets, latestVersion)
        };
      }
    ...
    try {
      const manifest = await requestJson(releaseManifestUrl);
      const latestVersion = normalizeVersion(manifest?.version);
      const archiveName = String(manifest?.asset?.name || "").trim();
      if (!latestVersion || !archiveName) {
        throw new Error("GitHub Release 更新清单缺少有效版本或安装包");
      }
      const assets = normalizeReleaseAssets([
        {
          ...
          browser_download_url: releaseManifestUrl
        },
        {
          id: 0,
          name: archiveName,
          size: Number(manifest?.asset?.size || 0),
          content_type: "application/zip",
          browser_download_url: `https://github.com/${repository}/releases/latest/download/${encodeURIComponent(archiveName)}`
        }
      ]);
      return {
        latestVersion,
        source: "release",
        sourceLabel: "GitHub Release",
        url: `${repositoryUrl}/releases/tag/v${latestVersion}`,
      ...
      releaseFailure = new Error(`${errorMessage(releaseFailure)}；${errorMessage(manifestError)}`);
    }
    ...
    try {
      ...
      return {
        ...
        assets: [],
        installable: false,
        installabilityReason: "RELEASE_NOT_PUBLISHED"
      };
    } catch (packageError) {
  ...
  async function performCheck(enabled) {
    ...
    try {
      ...
      return {
        ...
        assets: [],
        installable: false,
        installabilityReason: "UPDATE_CHECK_FAILED",
        checkedAt,
  ...
  async function check({ enabled = true, force = false } = {}) {
    ...
    if (!automaticCheckEnabled && !force) {
      return {
        ...
        assets: [],
        installable: false,
        installabilityReason: "AUTOMATIC_CHECK_DISABLED",
        checkedAt: null,
        error: ""
      };
    }
