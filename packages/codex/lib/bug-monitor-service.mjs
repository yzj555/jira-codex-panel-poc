...
import {
  ...
  buildIssuePrompt,
  isBugIssue
} from "@jira-workbench/core/public/prompt-builder.js";
...
export function createBugMonitorService({
  ...
  if (typeof loadIssueContext !== "function") throw new TypeError("Bug monitor requires loadIssueContext().");
  ...
  async function dispatch(state, issues, config, bindings, automationStatus) {
    ...
    try {
      if (item.threadId && item.turnId) {
        ...
        const contextualIssue = await loadIssueContext(issue);
        const payload = await analysisPayload(contextualIssue, config);
        ...
        item.lastError = "";
        await persist(state);
        await recoverLaunchedItem(item, contextualIssue, state.generation);
      }
