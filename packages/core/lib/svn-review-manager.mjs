...
async function fingerprintFiles(workingCopyRoot, selectedPaths) {
  let canonicalWorkingCopyRoot;
  try {
    canonicalWorkingCopyRoot = await realpath(workingCopyRoot);
  } catch (error) {
    if (error.code !== "ENOENT") throw error;
    canonicalWorkingCopyRoot = resolve(workingCopyRoot);
  }
  const fingerprints = [];
  for (const path of selectedPaths) {
    const absolutePath = resolve(canonicalWorkingCopyRoot, ...path.split("/"));
    try {
      const resolvedPath = await realpath(absolutePath);
      const outside = relative(canonicalWorkingCopyRoot, resolvedPath);
...
export function createSvnReviewManager({
  ...
  function normalizedWorkspaceContext(value, source = "service-binding") {
    ...
    return {
      ...
      projectScopeId: String(value.projectScopeId || value.scopeId || "").trim(),
  ...
  function getActiveOperations() {
    const activeStatuses = new Set(["dispatching", "running", "committing"]);
    return Array.from(reviews.values())
      .filter((review) => activeStatuses.has(review.status))
      .map((review) => ({
        kind: review.status === "committing" ? "svn_commit" : "codex_svn_review",
        issueKey: review.issue?.key || "",
        reviewId: review.id,
        status: review.status,
        message: review.status === "committing"
          ? `${review.issue?.key || "当前任务"} 正在提交 SVN`
          : `${review.issue?.key || "当前任务"} 正在执行 Codex SVN 审查`
      }));
  }
  ...
  function baselineKey(issueKey, threadId, projectScopeRoot = "") {
    const legacy = `${normalizeIssueKey(issueKey)}:${String(threadId || "").trim().toLowerCase()}`;
    return projectScopeRoot ? `${legacy}:${resolve(projectScopeRoot).toLowerCase()}` : legacy;
  }

  async function findBaseline(issueKey, threadId, workingCopyRoot, scopeRoot) {
    const state = await readBaselines();
    const baseline = state.baselines[baselineKey(issueKey, threadId, scopeRoot)]
      || state.baselines[baselineKey(issueKey, threadId)]
      || null;
    if (!baseline
  ...
  async function recordBaseline({
    ...
    const scopedKey = baselineKey(key, threadId, inspection.workingCopy.scopeRoot);
    const existing = state.baselines[scopedKey];
    if (preserveExisting && existing) {
      return {
        issueKey: key,
        threadId: String(threadId || "").trim(),
        capturedAt: existing.capturedAt,
        workingCopyRoot: existing.workingCopyRoot,
        projectScopeRoot: existing.projectScopeRoot,
        preExistingPaths: Array.isArray(existing.paths) ? existing.paths : [],
        preserved: true
      };
    }
    ...
    const ordered = Object.values(state.baselines)
      ...
      .slice(0, 500);
    state.baselines = Object.fromEntries(ordered.map((baseline) => [
      baselineKey(baseline.issueKey, baseline.threadId, baseline.projectScopeRoot || ""),
      baseline
    ]));
  ...
  function findLatestReview({ threadId, issueKey, workingCopyRoot = "" } = {}) {
    const normalizedThreadId = String(threadId || "").trim().toLowerCase();
    ...
    const review = Array.from(reviews.values()).reverse()
      ...
      .filter((candidate) => !workingCopyRoot
        || resolve(candidate.workingCopy?.root || ".") === resolve(workingCopyRoot))
