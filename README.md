...
新会话已经创建但首条消息发送失败时，会保留会话绑定和补充说明，并提供原地重试，不会自动再创建重复会话。
...
需求和 Bug 使用独立的精简模板。Jira 单号、标题、状态、人员、描述和附件清单由程序单独生成且只出现一次；模板只规定分析目标，避免首条消息被重复上下文和长篇规则撑大。附件内容通过真实文件传入，不会再次展开到消息正文。

每种模板都可以绑定一个额外 Codex Skill。需求默认不绑定额外技能；Bug 默认绑定 `ct-devops-tracer`。绑定 Skill 中与当前任务相关的工具、流程、证据、安全边界和输出格式优先；需求或 Bug 模板只补充 Skill 未覆盖的分析内容，内置 `jira-first-turn-analysis` 再补充仍未覆盖的首轮边界和证据规则。

所选 Skill 不存在或被禁用时，系统自动降级到分析模板、内置 Jira Skill 和已有 Jira/项目上下文，并在消息中给出简短提示。Skill 选择器会持续显示这条优先级规则，避免用户误认为模板会覆盖专业 Skill。
...
| 项目 Key | 内置通用 JQL 使用的项目 Key；使用自定义 JQL 或 Filter 时可留空 |
| 需求分析模板 | 独立控制需求首条消息正文；设置页仅显示摘要，点击“编辑”打开完整编辑器 |
| Bug 分析模板 | 独立控制 Bug 首条消息正文；自动 Bug 监控也使用这个模板 |
| 模板绑定技能 | 每种模板可单独选择 Codex 当前可用的 Skill；需求默认无，Bug 默认 `ct-devops-tracer` |
| 每个分区最大任务数 | 1～200，默认 100 |

设置面板按 Jira 连接、Codex 集成、数据同步、自动化通知、消息模板和高级配置分组。顶部和底部操作区固定，中间配置内容独立滚动，保存按钮始终可见。
...
- **内置通用 JQL**：按项目 Key、当前用户（经办人或协同处理人）和 Issue 类型自动查询；服务会先用当前项目可读取的 Issue 类型识别 Bug 类型，再为 Bug 面板加入类型条件、为需求面板排除这些类型；类型元数据不可读时仍可加载并由面板分类。
- **自定义 JQL**：直接使用该面板的 JQL；保存时服务会分别追加 `statusCategory != Done` 和 `statusCategory = Done`，用于活动面板与处理历史。
- **已有 Filter**：从当前用户可访问的 Jira Filter 中多选；服务查询时使用 `filter = <id>`，因此 Filter 在 Jira 中修改后，下一次同步会自动生效。
...
旧版本只有一个 JQL 配置时会自动迁移：默认配置保留原有 CT 规则，自定义 JQL 会同时作为需求和 Bug 面板的自定义来源。保存一次新的面板来源配置后，即可分别调整两个区域。
...
| `lib/codex-navigation.mjs` | 按会话 ID 调用 Codex 原生导航，发现可用 Skill，并读取、启动或中断当前会话 turn |
