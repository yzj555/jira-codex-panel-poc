...
export const LEGACY_CODEX_HOST_ID = "legacy-desktop";

export const LEGACY_CODEX_HOST_CAPABILITIES = Object.freeze({
  listSkills: true,
  listThreads: false,
  readThread: true,
...
export async function startCodexThreadTurn(threadId, prompt, options = {}) {
  ...
  try {
    ...
    throw withCodexErrorCode(error, "CODEX_LEGACY_HOST_UNAVAILABLE");
  }
  let state;
  try {
    ...
    throw withCodexErrorCode(error, "CODEX_LEGACY_PREFLIGHT_FAILED");
  }
  if (state.busy) {
...
export function createLegacyCodexHostAdapter(defaultOptions = {}) {
  const withDefaults = (options = {}) => ({ ...defaultOptions, ...options });
  return {
    id: LEGACY_CODEX_HOST_ID,

    getCapabilities() {
      return {
        hostOwner: LEGACY_CODEX_HOST_ID,
        capabilities: { ...LEGACY_CODEX_HOST_CAPABILITIES }
      };
    },
