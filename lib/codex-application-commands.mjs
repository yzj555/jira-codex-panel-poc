export const CODEX_APPLICATION_RUNTIME_OWNER = Object.freeze({
  APP_SERVER: "standalone-appserver",
  LEGACY_DESKTOP: "legacy-desktop"
});
...
const SAFE_MUTATION_FALLBACK_CODES = new Set([
  ...
  "CODEX_LEGACY_HOST_UNAVAILABLE",
  "CODEX_LEGACY_PREFLIGHT_FAILED"
]);
...
function attachmentRequirements(values) {
...
function appServerThreadState(payload, requestedThreadId) {
  ...
  return {
    ...
    lastTurnStatus,
    threadStatus,
    handoffReady: threadStatus === "notLoaded" || (!threadStatus && !busy),
...
export function createPanelCodexRuntimeAdapter({ request } = {}) {
  ...
  return {
    id: CODEX_APPLICATION_RUNTIME_OWNER.APP_SERVER,
    ...
    async startConversation(prompt, options = {}) {
      const payload = await request("/api/codex/app-server/analysis", {
        ...
        body: {
          ...
          desktopHandoff: options.desktopHandoff === true,
...
export function createCodexRuntimeSelector({
  runtimes = [],
  preferredRuntimeOwner = CODEX_APPLICATION_RUNTIME_OWNER.APP_SERVER,
  fallbackRuntimeOwner = CODEX_APPLICATION_RUNTIME_OWNER.LEGACY_DESKTOP
} = {}) {
...
export function createCodexApplicationCommands({ selector, legacyHost } = {}) {
  if (!selector || typeof selector.execute !== "function") {
    throw new TypeError("Codex Application Commands require a runtime selector.");
  }
  if (!legacyHost || !compactValue(legacyHost.id)) {
    throw new TypeError("Codex Application Commands require the Legacy Desktop Host for navigation and workspace resolution.");
  }
  ...
  return {
    ...
    resolveConversationId(threadId, options = {}) {
      ...
      return selector.execute("resolveConversationId", {
        ...
        runtimeOwner: provisional ? legacyHost.id : options.runtimeOwner,
        effect: "read",
    ...
    async createAnalysisConversation(prompt, options = {}) {
      ...
      if (!projectId && !explicitCwd) {
        return selector.execute("createAnalysisConversation", {
          ...
          effect: "mutation",
          allowFallback: false
        }, (runtime) => runtime.startConversation(prompt, options));
      }
      let cwd = explicitCwd;
      if (!cwd) {
        const target = await legacyHost.resolveConversationTarget(projectId, prompt, options);
        cwd = compactValue(target?.cwd);
      }
      const desktopManaged = options.desktopHandoff === true;
      return selector.execute("createAnalysisConversation", {
        capability: "createThread",
        runtimeOwner: desktopManaged ? legacyHost.id : options.runtimeOwner,
        // A desktop-managed creation atomically creates the conversation and
        // sends its first turn through Codex Desktop. App Server attachment
        // limits must not reroute that operation away from the desktop host.
        requirements: desktopManaged ? {} : requirements,
        effect: "mutation"
