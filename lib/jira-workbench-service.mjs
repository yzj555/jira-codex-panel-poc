...
export function createJiraWorkbenchService({
  ...
  async function fetchIssueWithParent(config, key) {
    const issue = await jiraClient.fetchIssue(config, key);
    const parentKey = String(issue?.parent?.key || "").trim().toUpperCase();
    if (!parentKey || parentKey === key) {
      return {
        ...issue,
        parentIssue: null,
        parentContext: { status: "none", key: "", message: "" }
      };
    }
    try {
      const parentIssue = await jiraClient.fetchIssue(config, parentKey);
      return {
        ...issue,
        parentIssue,
        parentContext: { status: "available", key: parentKey, message: "" }
      };
    } catch (error) {
      return {
        ...issue,
        ...
        parentContext: {
          status: "unavailable",
          key: parentKey,
          code: String(error?.code || "JIRA_PARENT_CONTEXT_UNAVAILABLE"),
          message: String(error?.message || "父级 Jira 任务暂时无法读取。")
        }
      };
    }
  }
  ...
  async function getIssue(issueKey) {
    ...
    return {
      issue: await fetchIssueWithParent(config, key),
  ...
  async function getAttachmentPreview(issueKey, attachmentId) {
    ...
    const issue = await jiraClient.fetchIssue(config, key);
    let metadata = (issue.attachments || [])
      .find((attachment) => String(attachment?.id || "") === id);
    if (!metadata && issue.parent?.key) {
      try {
        const parentIssue = await jiraClient.fetchIssue(config, issue.parent.key);
        metadata = (parentIssue.attachments || [])
          .find((attachment) => String(attachment?.id || "") === id);
      } catch {
        // Parent context is optional. A missing or inaccessible parent attachment
        // is reported through the same ownership error as a deleted attachment.
      }
    }
    if (!metadata) {
    ...
    return {
      issueKey: key,
      sourceIssueKey: String(metadata.sourceIssueKey || key),
      attachmentId: id,
