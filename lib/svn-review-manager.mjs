...
export function parseSvnReviewResult(content, reviewId, snapshotHash = "") {
  const markerIndex = String(content || "").lastIndexOf(REVIEW_RESULT_MARKER);
  if (markerIndex < 0) {
    throw new SvnReviewError("Codex 审核结果缺少结构化标记，请重新审核。", { code: "SVN_REVIEW_RESULT_MISSING" });
  }
  let parsed;
  try {
    parsed = JSON.parse(extractJsonObject(content, markerIndex));
  } catch {
...
最后必须输出标记 ${REVIEW_RESULT_MARKER}，标记后紧跟一个 JSON 对象，不要使用注释。格式：
{
  "reviewId": "${review.id}",
  "snapshotHash": "${review.snapshotHash}",
...
export function createSvnReviewManager({
  ...
  if (!sessionReader?.readContext || !sessionReader?.readReviewTurn) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (!blocked && codexReviewEnabled) {
          ...
          let observed = typeof sessionReader.readReviewTurn === "function"
            ? await sessionReader.readReviewTurn(review.threadId, lookupOptions)
            : null;
          if (observed) observed = { ...observed, threadId: review.threadId };
          if (!observed && typeof sessionReader.findReviewTurn === "function") {
  ...
  async function resolveWorkingCopy(threadId) {
    const context = await sessionReader.readContext(threadId);
    if (!context?.cwd) {
      throw new SvnReviewError("无法从当前 Codex 会话读取项目目录。", {
  ...
  async function inspect({
    ...
    const { context, root, scopeRoot, scopePath } = await resolveWorkingCopy(normalizedThreadId);
    ...
    let touchedFiles = [];
    if (includeAttribution && typeof sessionReader.readTouchedFiles === "function") {
      try {
  ...
  async function previewDiff({ threadId, path } = {}) {
    const selectedPath = normalizeSvnRelativePath(path);
    const inspection = await inspect({ threadId, selectedPaths: [selectedPath], includeAttribution: false });
  ...
  async function openExternalDiff({ threadId, path } = {}) {
    const selectedPath = normalizeSvnRelativePath(path);
    const inspection = await inspect({ threadId, selectedPaths: [selectedPath], includeAttribution: false });
    const change = inspection.changes.find((entry) => entry.path === selectedPath);
  ...
  async function recordBaseline({ threadId, issueKey, boundAt } = {}) {
    const key = normalizeIssueKey(issueKey);
    const inspection = await inspect({ threadId });
  ...
  async function captureSnapshot({
    ...
    const inspection = await inspect({ threadId, issue: normalized });
    if (checkUpdates) {
  ...
  async function createReview(input) {
    ...
    if (review.status === "prepared") {
      const conversationContext = typeof sessionReader.readConversationContext === "function"
        ? await sessionReader.readConversationContext(review.threadId).catch(() => null)
        : null;
  ...
  async function retryDispatch(reviewId, issue) {
    ...
    await persistReviews();
    return { review: publicReview(review), prompt: buildReviewPrompt(review) };
  }
  ...
  async function poll() {
    ...
    try {
      for (const review of reviews.values()) {
        if (review.status === "prepared") {
          const observed = await sessionReader.readReviewTurn(review.threadId, {
            reviewId: review.id,
            snapshotHash: review.snapshotHash,
            after: Math.max(0, Date.parse(review.createdAt) - 1_000)
          }).catch(() => null);
        ...
        if (review.status === "dispatching") {
          ...
          const observed = await sessionReader.readReviewTurn(review.auditThreadId, {
            reviewId: review.id,
            snapshotHash: review.snapshotHash,
            after: Math.max(0, Date.parse(review.createdAt) - 1_000)
          });
        ...
        const completion = await sessionReader.readReviewTurn(review.auditThreadId, {
          reviewId: review.id,
          snapshotHash: review.snapshotHash,
  ...
  async function verifyCommittedRevision(review, { revision = "", startedAt = "" } = {}) {
    ...
    const matches = entries.filter((entry) => (
      ...
      && entry.message.trim() === review.message.trim()
  ...
  async function commit(reviewId, { issue, confirmationToken } = {}) {
    ...
    try {
      ...
      if (verification.status !== "verified") {
        ...
        review.commitReceipt = { ...receipt, verification };
        review.error = `${result.exitCode === 0 ? "SVN 命令已结束" : `SVN 命令返回 ${result.exitCode}`}，但无法从日志唯一确认提交结果。请先人工核对，再选择重新核对或放弃本草稿。`;
  ...
  async function reconcileCommit(reviewId) {
    ...
    if (verification.status === "verified") {
      ...
      review.error = "仍未能从 SVN 日志唯一确认提交结果。可稍后再次核对，或人工确认仓库状态后放弃本草稿。";
    }
    await persistReviews();
    return publicReview(review);
  }
