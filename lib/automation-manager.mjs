...
function publicJob(job) {
  ...
  return {
    ...
    turnId: job.turnId || "",
    completionTracking: job.completionTracking || "legacy-session",
    status: job.status,
    pushStatus: job.pushStatus,
    startedAt: job.startedAt,
    completedAt: job.completedAt || "",
    completionSource: job.completionSource || "",
    monitorGeneration: Math.max(0, Number(job.monitorGeneration || 0)),
    error: job.error || ""
  };
}
...
export function createAutomationManager({
  ...
  async function register({ issue, threadId, turnId = "", startedAt, monitorGeneration = 0 }) {
    ...
    return mutate((state) => {
      ...
      const requestedGeneration = Math.max(0, Number(monitorGeneration || 0));
      if (existing
        && existing.status !== "failed"
        && Math.max(0, Number(existing.monitorGeneration || 0)) >= requestedGeneration) {
      ...
      const job = {
        ...
        completionTracking: String(turnId || "").trim() && typeof turnReader?.readTurnResult === "function"
          ? "app-server"
          : "legacy-session",
  ...
  async function markCompletion(issueKey, completion) {
    return mutate((state) => {
      ...
      job.completionSource = String(completion.source || "");
  ...
  async function readJobCompletion(job) {
    if (job.completionTracking === "app-server"
      && job.turnId
      && typeof turnReader?.readTurnResult === "function") {
      try {
        const observed = await turnReader.readTurnResult(job.threadId, job.turnId);
        if (observed && ["completed", "failed"].includes(observed.status)) return observed;
        return null;
      } catch {
        // Transient App Server errors leave the job running so the next poll
        // can recover through thread/read. They never switch a new job back to
        // rollout parsing implicitly.
        return null;
      }
    }
    if (typeof sessionReader?.readCompletion !== "function") return null;
    const legacy = await sessionReader.readCompletion(job.threadId, {
  ...
  async function poll() {
    ...
    try {
      ...
      for (const job of running) {
        const completion = await readJobCompletion(job);
