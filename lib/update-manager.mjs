...
function publicState(value, { managedInstallation, update } = {}) {
  ...
  const targetIsNewer = Boolean(value.targetVersion)
    && compareVersions(value.targetVersion, value.currentVersion) > 0;
  return {
    state: value.state,
    ...
    canDownload: Boolean(managedInstallation && update?.updateAvailable && update?.installable
      ...
      && (value.state !== "ready" || !targetMatches)),
    canCancelDownload: value.state === "downloading",
    canInstall: Boolean(managedInstallation && value.state === "ready" && targetIsNewer),
    canReset: ["failed", "cancelled", "rolled_back", "completed", "restart_required"].includes(value.state)
  };
}
...
export function createUpdateManager({
  ...
  async function reconcileInterruptedState(value, installation) {
    ...
    if (value.state === "installing" && !installStarting && !processAlive(value.updaterProcessId)) {
  ...
  async function startDownload(update) {
    if (downloadStarting) {
      ...
      operation = performDownload(update)
        .catch(async (error) => {
          ...
          await writeState({
            ...
            message: cancelled ? "更新下载已取消。" : "更新下载失败。",
  ...
  async function install({ confirmVersion, restartCodex = true } = {}) {
    if (installStarting) {
    ...
    if (current.state !== "ready" || !current.targetVersion || confirmed !== current.targetVersion) {
      throw new UpdateManagerError("安装前必须人工确认当前已下载的目标版本。", { code: "UPDATE_CONFIRMATION_REQUIRED", statusCode: 409 });
    }
    ...
    for (const path of [current.archivePath, current.manifestPath, updaterSource]) {
    ...
    await copyFile(updaterSource, stableUpdater);
    ...
    child.unref?.();
    await writeState({ ...next, updaterProcessId: Number(child.pid || 0) });
  ...
  async function reset(update = null) {
    const current = await readState();
    if (ACTIVE_STATES.has(current.state)) {
      throw new UpdateManagerError("更新操作正在执行，当前不能重置状态。", { code: "UPDATE_OPERATION_ACTIVE", statusCode: 409 });
    }
    await writeState(defaultState(normalizedCurrentVersion));
    return status(update);
  }
